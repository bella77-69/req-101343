...
Stock.createNewStock = (stockReqData, result) => {
  dbConn.query("INSERT INTO inventory SET ?", stockReqData, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
    } else {
...
Stock.getStockByAvailable = (available, result) => {
  dbConn.query(
    "Select * from inventory where available = ? ",
    available,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
...
Stock.getStockByRunningLow = (runningLow, result) => {
  dbConn.query(
    "Select * from inventory where runningLow = ? ",
    runningLow,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
...
Stock.getStockByOutOfStock = (outOfStock, result) => {
  dbConn.query(
    "Select * from inventory where outOfStock = ? ",
    outOfStock,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
